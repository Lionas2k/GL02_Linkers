...
GIFTParser.prototype.question = function (input) {
  ...
  // Reconstruire le raw GIFT à partir des données parsées
  q.raw = this.reconstructRaw(q);
...
GIFTParser.prototype.answerset = function (input) {
  ...
  while (input.length > 0 && input[0] !== "}") {
    ...
    if (sym === "=" || sym === "~") {
      ...
      let feedback = parts[1] ? parts[1].trim() : "";
...
/**
 * Reconstruit le format GIFT original à partir d'une Question parsée
 * @param {Question} question - La question parsée
 * @returns {string} Le texte GIFT reconstruit
 */
GIFTParser.prototype.reconstructRaw = function (question) {
  let raw = `::${question.id}:: ${question.enonce}`;

  // Pour les questions ESSAY, on peut avoir un answerset vide
  if (question.type === "ESSAY") {
    raw += `\n{ }`;
    return raw;
  }

  // Pour les autres types, construire l'answerset
  if (question.reponses && question.reponses.length > 0) {
    ...
    question.reponses.forEach((rep) => {
      ...
      // Questions MATCHING
      else if (question.type === "MATCH" && rep.assoc) {
        raw += `    =${rep.resp} -> ${rep.assoc}`;
      }
      // Questions MCQ et SA
      else {
        const prefix = rep.isCorrect ? "=" : "~";
        let answerLine = `    ${prefix}${rep.resp}`;
        
        // Ajouter le feedback s'il existe
        if (rep.feedback) {
          answerLine += `#${rep.feedback}`;
        }
    ...
    // Si aucune réponse, answerset vide
    raw += `\n{ }`;
  }
